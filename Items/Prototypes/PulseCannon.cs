...
namespace StarlightRiver.Items.Prototypes
{
    class PulseCannon : PrototypeWeapon
    {
        ...
        public override bool SafeUseItem(Player player)
        {
            ...
            for (int k = 0; k <= 100; k++)
            {
                Dust.NewDustPerfect(player.Center, ModContent.DustType<Dusts.Starlight>(), dir.RotatedByRandom(0.5f) * -Main.rand.NextFloat(200));
            }
            for (int k = 1; k <= 6; k++)
            {
                for (float n = 0; n <= 6.28f; n += 0.02f)
                {
                    ...
                    Dust.NewDustPerfect(player.Center + off.RotatedBy(dir.ToRotation()), ModContent.DustType<Dusts.Starlight>(), dir * -k * 40);
                }
            }
