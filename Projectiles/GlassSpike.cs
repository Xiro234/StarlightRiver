...
namespace StarlightRiver.Projectiles
{
    public class GlassSpike : ModProjectile, IDrawAdditive
    {
        ...
        public override void AI()
        {
            for (int k = 0; k <= 1; k++)
            {
                Dust d = Dust.NewDustPerfect(projectile.Center + projectile.velocity * 3, 264, (projectile.velocity * (Main.rand.NextFloat(-0.25f, -0.1f))).RotatedBy((k == 0) ? 0.4f : -0.4f), 0, default, 1f);
                d.noGravity = true;
            }
