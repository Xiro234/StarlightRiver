...
namespace StarlightRiver.NPCs.Boss.OvergrowBoss
{
    public class OvergrowBossFlail : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (npc.life <= 1)
            {
                Dust.NewDustPerfect(npc.Center, ModContent.DustType<Dusts.Gold2>(), Vector2.One.RotatedBy(LegendWorld.rottime * 4)); //dust when "destroyed"
            }
            ...
            if (npc.ai[2] == 1) //if zapped
            {
                ...
                if (npc.ai[1] % 5 == 0 && npc.ai[1] < 60)
                {
                    Helper.DrawElectricity(npc.Center, parent.npc.Center, ModContent.DustType<Dusts.Gold>(), 0.5f); //draw zap effects
                }
            ...
            if (npc.ai[0] == 1) //pick-upable
            {
                ...
                if (npc.velocity.Y == 0 && npc.velocity.X > 0.3f)
                {
                    npc.velocity.X -= 0.3f;
                }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            if (npc.ai[3] != 0)
            {
                for (float k = 0; k <= 1; k += 1 / (Vector2.Distance(npc.Center, parent.npc.Center) / 16))
                {
                    ...
                    if ((parent.npc.ai[2] == 3 || parent.npc.ai[0] == 4) && npc.velocity.Length() > 0)
                    {
                        pos += Vector2.Normalize(npc.Center - parent.npc.Center).RotatedBy(1.58f) * (float)Math.Sin(LegendWorld.rottime + k * 20) * 10;
                    }
