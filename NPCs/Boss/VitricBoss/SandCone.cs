...
namespace StarlightRiver.NPCs.Boss.VitricBoss
{
    class SandCone : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.ai[0] >= 72) //when this projectile goes off
            {
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            if(projectile.ai[0] <= 72) //draws the proejctile's tell 1 second before it goes off
            {
                Texture2D tex = ModContent.GetTexture("StarlightRiver/NPCs/Boss/VitricBoss/ConeTell");
                float alpha = ((projectile.ai[0] / 18) - ((float)Math.Pow(projectile.ai[0], 2) / 1296)) * 0.5f;
