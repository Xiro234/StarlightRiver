    ...
    class Seal : ModTile
    {
        ...
        public override void NearbyEffects(int i, int j, bool closer)
        {
            if (Main.tile[i, j].frameX == 0 && Main.tile[i, j].frameY == 0)
            {
                foreach (Player player in Main.player)
                {
                    ...
                    {
                        if (player.GetModPlayer<AbilityHandler>().smash.Active && !LegendWorld.SealOpen)
                        {
        ...
        public override void DrawEffects(int i, int j, SpriteBatch spriteBatch, ref Color drawColor, ref int nextSpecialDrawIndex)
        {
            if (Main.tile[i, j].frameX == 0 && Main.tile[i, j].frameY == 0)
            {
                ...
                if (!LegendWorld.SealOpen)
                {
                    spriteBatch.Draw(ModContent.GetTexture("StarlightRiver/Tiles/Void/SealClosed"), Seal - Main.screenPosition, Color.White);
                }
                else
                {
                    spriteBatch.Draw(ModContent.GetTexture("StarlightRiver/Tiles/Void/SealOpen"), Seal - Main.screenPosition, Color.White);
                }
            }
        }
    }
