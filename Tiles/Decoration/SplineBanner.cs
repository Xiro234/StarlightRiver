...
namespace StarlightRiver.Tiles.Decoration
{
    public abstract class SplineBanner : ModTile
    {
        ...
        public SplineBanner(string texturePath)
        {
            texture = texturePath;
        }
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            if (endpoint != Vector2.Zero) DrawSpline(spriteBatch, i, j, endpoint * 16, 1);
            else DrawSpline(spriteBatch, i, j, (Main.MouseWorld / 16).ToPoint16().ToVector2() * 16, 0.2f);
        }
        ...
        private void DrawSpline(SpriteBatch spriteBatch, int i, int j, Vector2 end, float colorMult)
        {
            ...
            if (max != 0)
                for (int k = 0; k < max; k++)
                {
                    ...
                    if(k > 0) spriteBatch.Draw(tex2, pos - Main.screenPosition, new Rectangle(k % 3 * 16, 0, 16, 16), color, (pos - oldPos).ToRotation() + 1.57f, tex2.Size() / 2, 1, 0, 0);
